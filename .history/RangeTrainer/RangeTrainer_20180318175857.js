...
class RangeTrainer extends Component {
  ...
  render() {
    ...
    return (
      <View className="rangeTrainer" style={styles.container}>
        <Scoreboard score={score} />
        <View>
          < title='Next Hand' onClick={this.newHand}/>
        </View>
        <View>Pos: {position}</View>
        <View>Hand: {hand}</View>
        ...
        {action && (
          <View>
            {action &&
              handSuccess && (
                <span style={{ backgroundColor: "green" }}>Correct action</span>
              )}

            {!handSuccess && (
              <span style={{ backgroundColor: "red" }}>Wrong action</span>
            )}
          </View>
        )}

        {wrongHands && (
          <View> {wrongHands.map(wrongHand => <View>{wrongHand}</View>)}</View>
        )}
      </View>
    );
  }
